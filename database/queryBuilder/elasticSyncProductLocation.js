...
module.exports = {
  ...
  fetchItemInventoryList: fetchItemInventoryList
}
...
function fetchLocationList(callBack) {
  const registerUserQuery = `select locationcode , countryCode , locationtypecode from locationinfo where
   status = 1 and countrycode = 'IN' and locationcode != ''`
...
function fetchItemInventoryList(lcoationList, countryCode, callBack) {
  const registerUserQuery = `SELECT
  li.locationcode,
  im.itemimage,
 ...
 icd.subcategorycountrycode,
  ...
  li.locationcode in ${lcoationList} AND im.countrycode = '${countryCode}'  and icd.countrycode = '${countryCode}'  AND im.activitycode IN ('ACT' , 'SEAS') 
  AND  inm.bucket_code = 5  AND inm.location_code in ${lcoationList} AND bd.expdate > NOW() AND im.itemcode NOT IN 
  (SELECT bd.itemcode FROM blockedlocation bd WHERE bd.itemcode = im.itemcode AND bd.locationcode in ${lcoationList}) 
